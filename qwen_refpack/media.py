...
def _orient_image(img, rotation: int = 0, mirror: bool = False):
    """Apply the reference's explicit orientation without interpolation.

    EXIF correction belongs to the caller because it establishes the source pixels the
    editor initially shows. Explicit clockwise rotation comes next, then a horizontal
    mirror in the rotated output's coordinate space. Crop is deliberately later.
    """
...
def load_image(path: str, crop=None, max_edge: int = 0, rotation: int = 0, mirror: bool = False):
    ...
    `rotation` / `mirror` are applied AFTER EXIF correction and BEFORE `crop`, so crop
    fractions refer to the explicit orientation the editor shows.
    ...
    with logs.timed("load_image", file=os.path.basename(path), crop=crop,
                    rotation=rotation or None, mirror=mirror or None,
                    cap=max_edge or None) as fields:
        img = Image.open(path)
        img = ImageOps.exif_transpose(img)
        img = img.convert("RGB")
        fields["src"] = f"{img.width}x{img.height}"
        img = _orient_image(img, rotation, mirror)
        if crop is not None:
...
def thumbnail_png(
    path: str, max_edge: int = 256, crop=None, at_seconds=None,
    rotation: int = 0, mirror: bool = False,
) -> bytes:
    ...
    loaders use, so the tile always previews exactly what the pack will emit. Explicit
    image rotation/mirroring happens before crop, matching load_image; video calls keep
    the default orientation and therefore retain their existing behavior.
